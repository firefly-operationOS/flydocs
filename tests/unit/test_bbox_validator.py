...
def test_good_bbox_for_plausible_text_line() -> None:
    field = ExtractedField(
        fieldName="name", fieldValueFound="John Doe", bbox=_bbox(0.10, 0.10, 0.30, 0.13)
    )
    _validate(field)
...
def test_poor_when_bbox_is_microscopic() -> None:
    ...
    field = ExtractedField(
        fieldName="x", fieldValueFound="x", bbox=_bbox(0.5, 0.5, 0.501, 0.501)
    )
    _validate(field)
...
def test_poor_when_aspect_ratio_extreme() -> None:
    ...
    field = ExtractedField(
        fieldName="bar", fieldValueFound="bar", bbox=_bbox(0.1, 0.5, 0.7, 0.51)
    )
    _validate(field)
    assert field.bbox.quality is BboxQuality.POOR
...
def test_empty_when_field_value_is_none() -> None:
    field = ExtractedField(
        fieldName="missing", fieldValueFound=None, bbox=_bbox(0.1, 0.1, 0.2, 0.13)
    )
    _validate(field)
...
def test_score_penalises_boxes_hugging_all_edges() -> None:
    ...
    hugging = ExtractedField(
        fieldName="a", fieldValueFound="a", bbox=_bbox(0.0, 0.0, 0.4, 0.05)
    )
    interior = ExtractedField(
        fieldName="b", fieldValueFound="b", bbox=_bbox(0.2, 0.2, 0.6, 0.25)
    )
    _validate(hugging)
    _validate(interior)
...
def test_recurses_into_array_field_rows() -> None:
    ...
    child = ExtractedField(
        fieldName="line", fieldValueFound="42", bbox=_bbox(0.1, 0.2, 0.3, 0.22)
    )
