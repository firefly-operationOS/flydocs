...
"""ASGI entry point for flydocs.

Loaded by ``pyfly run`` and by ``uvicorn flydocs.main:app``. Builds
the :class:`PyFlyApplication`, scans every ``@rest_controller`` /
``@configuration`` / ``@service`` / ``@controller_advice`` /
``@command_handler`` / ``@query_handler`` under ``flydocs``, and
...
from flydocs.web.openapi_override import install_openapi
...
_pyfly = PyFlyApplication(FlydocsApplication)
