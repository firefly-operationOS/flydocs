...
the domain-specific ones flydesk-idp throws so the API speaks
...
class ExceptionAdvice:
    ...
    async def job_not_ready(self, exc: JobNotReady) -> dict[str, Any]:
        problem = ProblemDetails(
            type="https://flydesk.dev/problems/job-not-ready",
            title="Job not ready",
            status=409,
            code="job_not_ready",
            detail=str(exc),
    ...
    async def job_not_cancellable(self, exc: JobNotCancellable) -> dict[str, Any]:
        problem = ProblemDetails(
            type="https://flydesk.dev/problems/job-not-cancellable",
    ...
    async def invalid_request(self, exc: ValueError) -> dict[str, Any]:
        problem = ProblemDetails(
            type="https://flydesk.dev/problems/invalid-request",
