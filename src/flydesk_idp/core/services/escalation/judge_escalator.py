...
Opt-in via:
  ...
  * ``options.escalation_threshold`` -- failure-rate trigger (0.0–1.0).
    Falls back to ``FLYDESK_IDP_ESCALATION_THRESHOLD``.
  * ``options.escalation_model`` -- the model id to escalate to. Falls
    back to ``FLYDESK_IDP_ESCALATION_MODEL``.
...
from flydesk_idp.interfaces.dtos.extract import EscalationInfo, ExtractionRequest
