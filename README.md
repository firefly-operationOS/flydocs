...
| **Post-extraction transforms** | Declarative entity resolution (dedupe entities across documents by a strong key — tax id / account no. — plus name variants) and free-form LLM transformations (closed-taxonomy normalisation, role mapping, provenance-aware consolidation with an optional parts-of-whole invariant, …). Per-document outputs land in the affected `documents[].field_groups[]`; cross-document outputs land in `request_transformations[]`. |
